...
class ConversationConfig:
    ...
    @classmethod
    def with_persistence(cls, data_dir: str = "data/conversations") -> "ConversationConfig":
        """创建支持持久化的配置"""
...
class MultiTurnConversationManager:
    ...
    def __init__(
        ...
        self.system_prompt = system_prompt or "你是一个智能助手，擅长帮助用户解决各种问题。"
        self._chain = None
    ...
    def get_history(self, session_id: str) -> list:
        ...
        return [{"role": msg.__class__.__name__.replace("Message", "").lower(), "content": msg.content} for msg in history.messages]
...
def create_conversation_manager(
    ...
    return MultiTurnConversationManager(llm=llm, config=config, system_prompt=system_prompt)
...
def demonstrate_persistence_switch():
    ...
    # 1. 内存模式（不支持持久化）
    print("1. 内存模式测试:")
    memory_manager = create_conversation_manager(
    ...
    persistent_manager = create_conversation_manager(
        enable_persistence=True,
        data_dir="data/conversations/demo"
    )
