## 一、基础配置信息

### 1. 密钥信息

```python
accessKeyId = "your_access_key_id"
accessKeySecret = "your_access_key_secret"
```

### 2. 知识库配置

```python
# 知识库ID
knowledge_base_id = "your_knowledge_base_id"

```

### 3. 业务空间配置

```python
# 业务空间ID
workspace_id = "your_workspace_id"
# agentKey
agentKey = "your_agent_key"
# 业务空间创建时间
workspace_create_time = "2025-03-24 23:38:04"

```

## 二、创建百炼客户端

### 3.1 安装依赖

```bash
uv pip install alibabacloud_bailian20231229==2.8.1
uv add alibabacloud_bailian20231229==2.8.1

```

> 参考文档：https://api.aliyun.com/api-tools/sdk/bailian?spm=a2c4g.11186623.0.0.679921f9n8F8og&version=2023-12-29&language=python-tea&tab=primer-doc
>

### 3.2 生成百炼客户端实例

```python
import os

from alibabacloud_tea_openapi import models as open_api_models
from alibabacloud_bailian20231229 import client as bailian_20231229_client

os.environ['ALIBABA_CLOUD_ACCESS_KEY_ID'] = "xxx"
os.environ['ALIBABA_CLOUD_ACCESS_KEY_SECRET'] = "xxx"

def create_client() -> bailian_20231229_client.Client:
    config = open_api_models.Config(
        access_key_id=os.environ.get('ALIBABA_CLOUD_ACCESS_KEY_ID'),
        access_key_secret=os.environ.get('ALIBABA_CLOUD_ACCESS_KEY_SECRET')
    )
    config.endpoint = 'bailian.cn-beijing.aliyuncs.com'
    return bailian_20231229_client.Client(config=config)

```

## 三、查询百炼知识库

### 4.1 百炼知识库查询通用方法

```python
from alibabacloud_bailian20231229 import models as bailian_20231229_models
from alibabacloud_tea_util import models as util_models

os.environ['WORKSPACE_ID'] = "llm-wh7mzfife6j35250"
os.environ['INDEX_ID'] = "iogsl3iryu"

def retrieve_index(client, workspace_id, index_id, query):
    """
    在指定的知识库中检索信息。

    参数:
        client (bailian_20231229_client.Client): 客户端（Client）。
        workspace_id (str): 业务空间ID。
        index_id (str): 知识库ID。
        query (str): 原始输入prompt。

    返回:
        阿里云百炼服务的响应。
    """
    headers = {}
    retrieve_request = bailian_20231229_models.RetrieveRequest(
        index_id=index_id,
        query=query
    )
    runtime = util_models.RuntimeOptions()
    return client.retrieve_with_options(workspace_id, retrieve_request, headers, runtime)

```

### 4.2 分段查询知识库

```python
os.environ['WORKSPACE_ID'] = "llm-***"
os.environ['INDEX_ID'] = "i***u"

def query_rag_from_bailian(query):
    bailian_client = create_client()
    rag = retrieve_index(bailian_client, os.environ['WORKSPACE_ID'], os.environ['INDEX_ID'], query)

    result = ""
    index = 1
    for data in rag.body.data.nodes:
        result += f"""第{index}段知识：
    {data.text}
    --
    """
        index += 1

    print(result)
    return result

```

## 四、智能体知识库工具🤖

### 步骤1：封装mcp工具

```python
from mcp.server.fastmcp import FastMCP

mcp = FastMCP()

@mcp.tool(name="query_rag_from_bailian", description="从百炼平台查询知识库，传入需要查询的知识关键字即可")
def query_rag_from_bailian(query):
    # 实现同上

if __name__ == '__main__':
    mcp.run(transport="stdio")

```

### 步骤2：生成langchain工具

```python
from app.utils.mcp import create_mcp_stdio_client

async def get_stdio_rag_tools():
    params = {
        "command": "python",
        "args": [
            "/Users/sam/Xiaoluyy/ai/coding_agent/app/mcp/rag.py",
        ],
    }

    client, tools = await create_mcp_stdio_client("rag", params)

    return tools

```